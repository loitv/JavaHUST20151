...
public class ClientDemo extends JFrame implements ActionListener {
	...
	public static void main(String[] args) {
		...
		try {
			...
			sB = new Socket(ip, 2222); // Kết nối dựa trên ip vừa nhập và cổng
										// mặc định
			BufferedReader nhan = new BufferedReader(new InputStreamReader(sB.getInputStream())); // Luồng nhận tin
			while ((chuoi = nhan.readLine()) != null) // Kiểm tra xem có tin để
														// nhận hay không?
			{
	...
	public ClientDemo() {
		...
		JScrollPane sp = new JScrollPane(content); // Cho phép scrolling khi nội
													// dung quá dài
		...
		toName = new JTextField("Nhập tên bạn"); // Định nghĩa nội dung mặc định
													// cho khung
		...
		setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE); // Thoát khi đóng chương
														// trình
	}
